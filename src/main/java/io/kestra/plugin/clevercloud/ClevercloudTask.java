...
@Plugin(
    examples = {
        @io.kestra.core.models.annotations.ClevercloudTask(
            title = "Simple revert",
            code = { "format: \"Text to be reverted\"" }
        )
    }
)
public class ClevercloudTask extends Task implements RunnableTask<ClevercloudTask.Output> {
    @Schema(
    ...
    @Override
    public ClevercloudTask.Output run(RunContext runContext) throws Exception {
